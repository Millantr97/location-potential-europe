...
function buildReport(id){
  ...
  const s1=`<section class="rslide rcover"><div class="rslide-in">
    ...
    <div class="rcover-src">Dataset built ${META.built} · OpenStreetMap ${META.osm_date} · ${META.numbat} · ${META.census} ${HAS_CRIME?` · ${CITY.texts.police||"Police"} ${META.crime_window}`:""} · ${CITY.eu?"Modelled city-level rents":((CITY.id==="glasgow"||CITY.id==="edinburgh")?"Scottish Assessors valuation roll":"VOA "+(CITY.texts.voaYear||"2023"))}</div>
  </div></section>`;
  ...
  <div class="rcols2">
    ...
    <div class="rcard"><div class="rsub">Who lives around it · ${esc(s.lsoa.name)} ${chip("ctx")}</div>
      ${kv("Usual residents ("+(CITY.texts.resCensus||"Census 2021")+")",Math.round(s.lsoa.residents).toLocaleString("en-GB"))}
      ...
      ${s.lsoa.pct_nonuk!=null?kv(CITY.texts.bornAbroad||"Born outside the UK",s.lsoa.pct_nonuk.toFixed(1)+"%"):""}${kv("Ethnic diversity index (0-1)",s.lsoa.diversity.toFixed(2))}
      ${topEth}
  ...
  <div class="rcols2">
    ...
    <div class="rcard"><div class="rsub">Sources</div>
      ...
      <div class="rkv2"><span class="rl">Residents ${chip("ctx")}</span><span class="rv">${CITY.eu?"Census 2021 1 km grid (Eurostat)":"Census 2021 LSOA via Nomis"}</span></div>
      ...
      <div class="rmini">Resolution honesty: station counts are not pavement footfall; ${CITY.eu?"the census grid cell describes residents not visitors":"LSOA describes residents not visitors"}; OSM counts are lower bounds; modelled layers are the ones to override with your own counts.</div>
    </div>
  </div>
