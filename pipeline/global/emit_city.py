...
def build(i,name,la,lo,lvl,us):
 ...
 d=demo(la,lo); stype='transport_hub' if lvl=='area' else ('high_street' if len(us)>=40 else 'side_street');rm=1450; est=round(rm*(1.18 if stype=='transport_hub' else 1.1 if stype=='high_street' else .92))
 return {'id':slug(name)+'-'+str(i),'name':name,'borough':C['name'],'zone':'city','lat':round(la,5),'lng':round(lo,5),'stype':stype,'lvl':lvl,'weak':False if lvl=='area' else True,'anchors':([{'station':name.replace(' station area',''),'mode':'MODELLED','annual':annual,'src':'modelled','days_modelled':True}] if lvl=='area' else []),'flow':{'annual_total':annual,'days':days,'modelled_from':name if lvl=='area' else None,'share':None,'weekend_share':.25,'weekend_ratio_norm':.5,'fri_sat_norm':.5,'day_rel':{'mon':.95,'mid':1,'fri':1.06,'sat':1.12,'sun':.68}},'transport':{'stations_900m':1 if lvl=='area' else 0,'names':[name.replace(' station area','')] if lvl=='area' else []},'osm':co,'lsoa':d,'crime':None,'crime_per1000':None,'rent':{'retail_rv_m2':rm,'office_rv_m2':900,'basis':'modelled','est_rent_m2':est},'model':{'spend_est':round(42+22*d['pct_prof']/100),'office_skew':round(.25+.5*d['pct_prof']/100,2),'rhythm':{'early':.12,'midday':.28,'afternoon':.23,'evening':.28,'late':.09}},'_units':us}
segs=[]
