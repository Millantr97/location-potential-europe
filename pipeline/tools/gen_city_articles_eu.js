...
global.window={};
const fs=require('fs');
const R='/home/sandbox/europe/repo';
...
const genSrc=fs.readFileSync('/home/sandbox/london-location-lens/pipeline/gen_articles.js','utf8');
...
const SCOT=false;
const geoUnit='census grid cell (1 km)';
const rentSrc='city-level modelled rents';
...
for(const a of ARTICLES){
  ...
  <div class="ai-main">
    ...
    <div class="ai-figs">
      ...
      <span class="ai-fig">Weekly station flow <b>${fmt(fl)}</b><span class="chip ${(s.weak||CITY.eu)?'mod':'obs'}">${(s.weak||CITY.eu)?'MODELLED':'OBSERVED'}</span></span>
...
<meta name="description" content="Data-driven ranking of the 10 best ${cn} streets to open ${esc(a.noun)}: fit scores, modelled revenue, modelled station flows and recorded competition, with honest labels on every figure.">
