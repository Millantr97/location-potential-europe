...
function renderConcept(){
  ...
  <div class="cg-card money-card"><h3>Money</h3>
    ${sliderField(CITY.eu?"Rent tolerance (typical rent, "+CUR+"/m²/yr)":"Rent tolerance (rateable-value proxy, £/m²/yr)","rent",100,1500,25,v=>money(v)+"/m²")}
    ${rentReference()}
  </div>
...
function selectSegment(id,scroll){
  ...
  <div class="dp-cols">
    ...
    <div class="ev-card"><h4>Who lives around it · ${s.lsoa.name}${chipFor("ctx")}</h4>
      ...
      <div class="ev-line"><span class="lv">${CITY.texts.ageYoung||"Aged 20-39"}</span><span class="rv">${s.lsoa.pct20_39.toFixed(1)}%</span></div>
      <div class="ev-line"><span class="lv">${CITY.texts.studentsLbl||"Students (16+)"}</span><span class="rv">${s.lsoa.pct_students.toFixed(1)}%</span></div>
      <div class="ev-line"><span class="lv">${CITY.texts.profLbl||"Professional / managerial jobs"}</span><span class="rv">${s.lsoa.pct_prof.toFixed(1)}%</span></div>
      <div class="ev-line"><span class="lv">${CITY.texts.bornAbroad||"Born outside the UK"}</span><span class="rv">${s.lsoa.pct_nonuk.toFixed(1)}%</span></div>
...
function renderMethod(){
  ...
  <div class="m-card"><h4>Movement &amp; transport${chipFor("obs")}</h4>
    ...
    ${CITY.eu?`<p>Flow here is MODELLED from the system-level ridership named in each segment panel - no stop-level counts are published to link to. Station locations and all venue counts come from <a href="https://www.openstreetmap.org/about">OpenStreetMap</a>.</p>`:(CITY.texts.movement?`<p><a href="https://dataportal.orr.gov.uk/statistics/usage/estimates-of-station-usage">ORR dataportal - Estimates of Station Usage 2024-25</a></p>`:`<p><a href="https://crowding.data.tfl.gov.uk/Annual%20Station%20Counts/2024/AC2024_AnnualisedEntryExit_Public.xlsx">crowding.data.tfl.gov.uk - AC2024 Annualised Entry/Exit</a></p>`)}
  ...
  <div class="m-card"><h4>Residents${chipFor("ctx")}</h4>
    ...
    ${CITY.eu?`<p><a href="https://ec.europa.eu/eurostat/web/gisco/geodata/grids">eurostat - Census 2021 population grid (GEOSTAT/GISCO)</a></p>`:`<p><a href="https://www.nomisweb.co.uk/sources/census_2021_bulk">nomisweb.co.uk - Census 2021 bulk downloads</a></p>`}
  ...
  <div class="m-card"><h4>Occupancy cost${chipFor("ctx")}</h4>
    ...
    ${CITY.eu?`<p><a href="https://www.cushmanwakefield.com/en/insights/main-streets-across-the-world">Cushman &amp; Wakefield - Main Streets Across the World (rent anchor)</a></p>`:`<p><a href="https://www.gov.uk/government/statistics/non-domestic-rating-stock-of-properties-including-business-floorspace-2023">gov.uk - NDR business floorspace 2023</a></p>`}
  ...
  <div class="m-card"><h4>Observed city pulse${chipFor("obs")}</h4>
    <p>This published city dataset contains <b>${UNITS.length.toLocaleString("en-GB")}</b> mapped commercial premises, including <b>${UNITS.filter(u=>u[6]).length.toLocaleString("en-GB")}</b> named venues, across <b>${SEGS.length.toLocaleString("en-GB")}</b> scored segments. It also records <b>${SEGS.reduce((n,s)=>n+(s.transport?.stations_900m||0),0).toLocaleString("en-GB")}</b> segment-to-station links and the local culture, parks, pharmacies, vets and everyday-service counts shown in each evidence panel.</p>
    <p>Source: OpenStreetMap extract ${META.osm_date}. These are OBSERVED mapped features, not estimates of total businesses. Duplicate station links across overlapping segments are retained because they describe access from each candidate zone.</p></div>
  <div class="m-card"><h4>Concept trends${chipFor("obs")}${chipFor("mod")}</h4>
    <p>The Trends section leads with search interest: Google Trends yearly mean index for a keyword matching each concept, ${CITY.country_name||"United Kingdom"}, rescaled across batches against a shared anchor ("restaurant"). ESTIMATED attention, not shops or sales. Alongside it, one OBSERVED supply figure: every venue currently recorded in the concept's category (all cafés, all restaurants, all fast food, pubs/bars/nightclubs, grocery and food shops, gyms, coworking) inside the same ${CITY.region} bounding box this map uses, from OpenStreetMap.</p>
